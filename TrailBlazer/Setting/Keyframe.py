...
def get_dyn_keyframe(prompt, fix_seed=False):
    ...
    # note: keyframing here does back and forth movement
    ...
    keyframe = []
    for i in range(num_keyframes):
        frame_dict = {"bbox_ratios": bbox_ratios[i], "frame": frames[i], "prompt": prompt}
        if i==0:
            # keep track of randomly selected choices
            frame_dict["bbox_motion_choice"] = choice
            frame_dict["num_keyframes_choice"]  = num_keyframes
            
        keyframe.append(frame_dict)
