...
from .Utils_origin import (initiailization, keyframed_bbox, keyframed_prompt_embeds, use_dd, 
                           use_dd_temporal)
# avoid repetition
from .Utils import extract_overlap_maps
...
def text_to_video_sd_pipeline_call(
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        ...
        for t_idx, t in enumerate(timesteps):
            ...
            # pass in general variables
            use_dd(self.unet, timestep=t_idx, wandb_log=wandb_log, 
                       output_opt_viz_path=output_opt_viz_path, use=None)
            use_dd_temporal(self.unet, timestep=t_idx, wandb_log=wandb_log, 
                       output_opt_viz_path=output_opt_viz_path, use=None)

            # was_edited=False
            # predict the noise residual
            if i < (num_dd_spatial_steps):
                use_dd(self.unet, use=True)
            ...
            if i < (num_dd_temporal_steps):
                use_dd_temporal(self.unet, use=True)
                # was_edited = True

            # print(f"t_idx ONE {t_idx} i {i}")
            # example_step =  self.unet.mid_block.attentions[0].transformer_blocks[0].attn2.processor.timestep
            # print(f"example_step {example_step}")

            # if t_idx==7:

            # TODO: show that timestep from the processor
            # confirm that the last update from step 4 is being used at beginning step 5
            # override processor here directly
            # how is timestep used internally, determine effect of MAJOR/MINOR bug?
            ...
            use_dd(self.unet, use=False)
            use_dd_temporal(self.unet, use=False)


            # NON-EDIT STEPS
            # ------------------------------
            # after total forward editing is done (the last one)
            if t_idx==(num_dd_spatial_steps-1) and t_idx==(num_dd_temporal_steps-1):
                
                # print(f'NON-EDIT: t_idx {t_idx}') 
                if bundle['overlay_maps']:
