...
class CrossAttnPainter:
    ...
    def plot_spatial_attn(self):
        # pdb.set_trace()
        arr = (
            self.pipe.unet.up_blocks[1].attentions[0].transformer_blocks[0].attn2.processor._cross_attention_map_str
            # self.pipe.unet.up_blocks[1].attentions[0].transformer_blocks[0].attn2.processor._cross_attention_map
        )
        assert self.bundle['width']==512, 'need a dynamic shape version'
        heads = self.pipe.unet.up_blocks[1].attentions[0].transformer_blocks[0].attn2.heads
        arr = torch.transpose(arr, 1, 3)
        arr = nnf.interpolate(arr, size=(64, 64), mode='bicubic', align_corners=False)
        arr = torch.transpose(arr, 1, 3)
        arr = arr.cpu().numpy()
        
        arr = arr.reshape(25, heads, 64, 64, 77)
        arr = arr.mean(axis=1)
        n = arr.shape[0]
        for i in range(n):
            filename = f"{self.root}/spatialca.{i:04d}.png"
            plt.clf()
    ...
    def plot_temporal_attn(self):
        ...
        arr = self.pipe.unet.mid_block.temp_attentions[0].transformer_blocks[0].attn2.processor._cross_attention_map_str
        import matplotlib.pyplot as plt
        import torch.nn.functional as nnf
        
        # arr = (self.pipe.unet.up_blocks[2].temp_attentions[1].transformer_blocks[0].attn2.processor._cross_attention_map_str)
        # arr = (self.pipe.unet.up_blocks[2].temp_attentions[1].transformer_blocks[0].attn2.processor.cross_attention_map)

        # pdb.set_trace()
        if arr!=None:
            #arr = self.pipe.unet.transformer_in.transformer_blocks[0].attn2.processor.cross_attention_map
            ...
            n = arr.shape[-1]
            for i in range(n-2):
                filename = f"{self.root}/tempcaiip2.{i:04d}.png"
                plt.clf()
...
def plot_latent_noise(latents, mode):
    ...
    for i in range(latents.shape[0]):
        ...
        filepath = f"{self.root}/out.{i:04d}.jpg"
        img.save(filepath)
        ...
        filepath = f"{self.root}/hist.{i:04d}.jpg"
